#!/bin/bash
set -e
cd ~/termux-mirror/scripts/
owner=${1:-termux-pod}
repo=${2:-science-packages}
termux_repo=${3:-termux-science}
#dists=` echo ${3} | cut -d'-' -f2`
dists=${4:-science}
tag=deb
pkg_url="https://packages.termux.org/apt/termux-science/"
...
fetcher() {
	...
	curl -f  https://packages.termux.org/apt/${termux_repo}/dists/${dists}/stable/binary-aarch64/Packages -Lo $aarch64
	curl -f  https://packages.termux.org/apt/${termux_repo}/dists/${dists}/stable/binary-arm/Packages -Lo $arm
	curl -f  https://packages.termux.org/apt/${termux_repo}/dists/${dists}/stable/binary-i686/Packages -Lo $i636
	curl -f  https://packages.termux.org/apt/${termux_repo}/dists/${dists}/stable/binary-x86_64/Packages -Lo $x86
